...
MYISATargetLowering::MYISATargetLowering(const TargetMachine &TM,
  ...
  // ======================= Stage 1 操作合法性 =========================
  // 原则：宁可保守。没配置的默认是 Legal，意思是"我有对应指令且 .td 里有
  // 匹配模式"——如果其实没有，编译时会报 "Cannot select"。
  // 所以 Stage 1 把还没实现的一律标成 Expand，让 LLVM 自己想办法。

  // 布尔值用 0/1 表示（我们的 CMP 写的是差值，但比较结果由分支指令解释）
  ...
  // ---- 全局变量地址：模板已实现 LowerGlobalAddress ----
  setOperationAction(ISD::GlobalAddress, MVT::i32, Custom);

  // ---- 硬件没有的运算 ----
  // MUL：ISA 里有操作码，但 Stage 1 的 alu.v 还没实现，先让 LLVM 拆开
  ...
  // 移位类：ISA 有 SHL/SHR/SAR，但 Stage 1 的 alu.v 和 .td 都还没接
